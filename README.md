...
<tr>
    ...
    <td>
        `POST /cart/items`<br>
        Тело запроса: `{ "id": 1, "name": "Item 1", "price": 15, "quantity": 10 }`
    </td>
    <td>
        `201 Created`
    </td>
...
<tr>
    ...
    <td>`Cart::updateItem()`</td>
    <td>
        `PUT /cart/items/:id`<br>
        Тело запроса: `{ "id": 1, "name": "Item 1", "price": 15, "quantity": 10 }`
    </td>
    <td>
        `204 No Content`
...
1) Реализуйте все методы модели без выполнения HTTP-запросов. Кроме, разумеется, `Cart::load()`, в него можно поставить фейковые данные.
...
### Задача 7 (*)
Напишите класс `EnhancedPromise`, который наследуется от `Promise`, и расширяет его методом `some()`. Метод принимает первым параметром массив промисов, а вторым параметром число (`count`). 

Метод вовзращает промис, который разрешится, если из переданных в массиве промисов разрешилось не менее `count`, или будет отклонен в противном случае. Если промис разрешился, то возвращается массив из первых `count` ответов (очередность в порядке разрешения).

```javascript
EnchancedPromise.some(["url1", "url2", "url3"], 2)
